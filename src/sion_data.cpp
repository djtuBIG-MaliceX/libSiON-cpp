...
std::shared_ptr<SiOPMWavePCMData> SiONData::set_pcm_wave(int p_index, const Variant &p_data, double p_sampling_note, int p_key_range_from, int p_key_range_to, int p_src_channel_count, int p_channel_count) {
	std::shared_ptr<SiOPMWavePCMTable> pcm_table = get_pcm_voice(p_index)->get_wave_data();
...
void SiONData::set_pcm_voice(int p_index, const std::shared_ptr<SiONVoice> &p_voice) {
	// Size is expected to be power of 2.
	_pcm_voices[p_index & (_pcm_voices.size() - 1)] = p_voice;
}

std::shared_ptr<SiOPMWaveSamplerData> SiONData::set_sampler_wave(int p_index, const Variant &p_data, bool p_ignore_note_off, int p_pan, int p_src_channel_count, int p_channel_count) {
...
void SiONData::set_sampler_table(int p_bank, const std::shared_ptr<SiOPMWaveSamplerTable> &p_table) {
	// Size is expected to be power of 2.
	_sampler_tables[p_bank & (_sampler_tables.size() - 1)] = p_table;
}
