...
std::vector<int> TranslatorUtil::_split_data_string(const std::shared_ptr<SiOPMChannelParams> &p_params, std::string p_data_string, int p_channel_param_count, int p_operator_param_count, const std::string &p_command) {
	...
	for (int i = 1; i <= SiOPMChannelParams::MAX_OPERATORS; i++) {
		if (string_data.size() == (p_channel_param_count + p_operator_param_count * i)) {
			...
			std::vector<int> data;
			data.resize(string_data.size()); // TODO zeroed
			for (int i = 0; i < string_data.size(); i++) {
				data[i] = string_data[i].to_int();
			}
			return data;
		}
	}
...
#define EXTRACT_ARGUMENT_POS(m_index, m_default)                                                                                  \
	...
	for (int i = 0; i < settings.size(); i++) {
		...
		if (command == "@f") {
			...
			params->master_volumes[0] = EXTRACT_ARGUMENT_MOD(0, 0.0078125, 0.5);
			params->master_volumes[1] = EXTRACT_ARGUMENT_MOD(1, 0.0078125, 0);
			params->master_volumes[2] = EXTRACT_ARGUMENT_MOD(2, 0.0078125, 0);
			params->master_volumes[3] = EXTRACT_ARGUMENT_MOD(3, 0.0078125, 0);
			params->master_volumes[4] = EXTRACT_ARGUMENT_MOD(4, 0.0078125, 0);
			params->master_volumes[5] = EXTRACT_ARGUMENT_MOD(5, 0.0078125, 0);
			params->master_volumes[6] = EXTRACT_ARGUMENT_MOD(6, 0.0078125, 0);
			params->master_volumes[7] = EXTRACT_ARGUMENT_MOD(7, 0.0078125, 0);
