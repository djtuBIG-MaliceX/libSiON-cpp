/***************************************************/
/* Part of GDSiON software synthesizer             */
/* Copyright (c) 2024 Yuri Sizov and contributors  */
/* Provided under MIT                              */
/***************************************************/

#include "transformer_util.h"

#include "chip/siopm_ref_table.h"
#include "chip/wave/siopm_wave_table.h"

#include <vector>
#include <memory>
#include <algorithm>

void TransformerUtil::_amplify_log_data(std::vector<int> *r_src, int p_gain) {
	int gain = p_gain & ~1;

	for (int i = 0; i < r_src->size(); i++) {
		r_src[i] -= gain;
	}
}

std::vector<int> TransformerUtil::transform_pcm_data(std::vector<double> p_source, int p_src_channel_count, int p_channel_count, bool p_maximize) {
	std::vector<int> result;
	int max_gain = SiOPMRefTable::LOG_TABLE_BOTTOM;

	if (p_src_channel_count == p_channel_count || p_channel_count == 0) {
		int target_size = p_source.size();
		result.resize(target_size); // TODO zeroed

		for (int i = 0; i < target_size; i++) {
			result[i] = SiOPMRefTable::calculate_log_table_index(p_source[i]);

			if (result[i] < max_gain) {
				max_gain = result[i];
			}
		}

	} else if (p_src_channel_count == 2) { // p_channel_count == 1
		int target_size = p_source.size() >> 1;
		result.resize(target_size); // TODO zeroed

		int j = 0;
		for (int i = 0; i < target_size; i++) {
			double value = p_source[j];
			j++;
			value += p_source[j];
			j++;

			result[i] = SiOPMRefTable::calculate_log_table_index(value * 0.5);

			if (result[i] < max_gain) {
				max_gain = result[i];
			}
		}

	} else { // p_src_channel_count == 1, p_channel_count == 2
		int target_size = p_source.size();
		result.resize(target_size << 1); // TODO zeroed

		int j = 0;
		for (int i = 0; i < target_size; i++) {
			result[j] = SiOPMRefTable::calculate_log_table_index(p_source[i]);
			result[j + 1] = result[j];
			j += 2;

			if (result[j] < max_gain) {
				max_gain = result[j];
			}
		}
	}

	if (p_maximize && max_gain > 1) {
		_amplify_log_data(&result, max_gain);
	}
	return result;
}

std::vector<double> TransformerUtil::transform_sampler_data(std::vector<double> p_source, int p_src_channel_count, int p_channel_count) {
	std::vector<double> result;

	if (p_src_channel_count == p_channel_count || p_channel_count == 0) {
		return p_source;

	} else if (p_src_channel_count == 2) { // p_channel_count == 1
		int target_size = p_source.size() >> 1;
		result.resize(target_size); // TODO zeroed

		for (int i = 0, j = 0; i < target_size; i++, j += 2) {
			result[i] = (p_source[j] + p_source[j + 1]) * 0.5;
		}

	} else { // p_src_channel_count == 1, p_channel_count == 2
		int target_size = p_source.size();
		result.resize(target_size << 1); // TODO zeroed

		for (int i = 0, j = 0; i < target_size; i++, j += 2) {
			result[j] = p_source[i];
			result[j + 1] = p_source[i];
		}

	}

	return result;
}

std::vector<double> TransformerUtil::wave_color_to_vector(uint32_t p_color, int p_wave_type) {
	std::vector<double> result;

	int bits = 0;
	for (int length = SiOPMRefTable::SAMPLING_TABLE_SIZE >> 1; length != 0; length >>= 1) {
		bits++;
	}
	result.resize(1 << bits); // TODO zeroed

	double bars[7];
	uint32_t color = p_color;
	for (int i = 0; i < 7; i++) {
		bars[i] = (color & 15) * 0.0625;
		color >>= 4;
	}

	int barr[7] = { 1,2,3,4,5,6,8 };
	int (&log_table)[SiOPMRefTable::LOG_TABLE_SIZE * 3] = SiOPMRefTable::get_instance()->log_table;
	std::shared_ptr<SiOPMWaveTable> wave_table = SiOPMRefTable::get_instance()->get_wave_table(p_wave_type + (color >> 28));
	int envelope_top = (-SiOPMRefTable::ENV_TOP) << 3;

	double value_max = 0;

	bits = SiOPMRefTable::PHASE_BITS - bits;
	int step = SiOPMRefTable::PHASE_MAX >> bits;
	for (int i = 0; i < SiOPMRefTable::PHASE_MAX; i += step) {
		int j = i >> bits;

		result[j] = 0;
		for (int mult_idx = 0; mult_idx < 7; mult_idx++) {
			int gain_idx = (((i * barr[mult_idx]) & SiOPMRefTable::PHASE_FILTER) >> wave_table->get_fixed_bits());
			int gain = wave_table->get_wavelet()[gain_idx] + envelope_top;

			result[j] += log_table[gain] * bars[mult_idx];
		}

		double value = std::abs(result[j]);
		if (value_max < value) {
			value_max = value;
		}
	}

	value_max = std::max(value_max, 8192.0);
	double value_coef = 1.0 / value_max;
	for (int i = 0; i < result.size(); i++) {
		result[i] *= value_coef;
	}

	return result;
}
