...
std::vector<double> SiOPMWaveBase::_extract_wave_data(const std::shared_ptr<AudioStream> &p_stream, int *r_channel_count) {
	if (p_stream.is_null()) {
		return std::vector<double>();
	}

	std::shared_ptr<AudioStreamWAV> wav_stream = p_stream;
	if (wav_stream.is_valid()) {
		...
		if (data_format != AudioStreamWAV::FORMAT_8_BITS && data_format != AudioStreamWAV::FORMAT_16_BITS) {
			//ERR_FAIL_V_MSG(std::vector<double>(), vformat("SiOPMWaveBase: Unsupported WAV file format (%d).", data_format));
		}
	...
	//ERR_FAIL_V_MSG(std::vector<double>(), "SiOPMWaveBase: Unsupported audio stream format.");
}
