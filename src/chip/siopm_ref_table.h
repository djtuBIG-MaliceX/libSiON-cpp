...
class SiOPMRefTable {
	...
	// Custom wave tables.
	std::vector<Ref<SiOPMWaveTable>> _custom_wave_tables;
	// Overriding custom wave tables.
	std::vector<Ref<SiOPMWaveTable>> _stencil_custom_wave_tables;
	// PCM voices.
	std::vector<Ref<SiMMLVoice>> _pcm_voices;
	// Overriding PCM voices.
	std::vector<Ref<SiMMLVoice>> _stencil_pcm_voices;
...
public:
	...
	HashMap<sion::String, Ref<SampleData>> sound_reference;
	...
	// PG wave tables.
	std::vector<Ref<SiOPMWaveTable>> wave_tables;
	// PG wave tables without any waves.
	Ref<SiOPMWaveTable> no_wave_table;
	Ref<SiOPMWaveTable> no_wave_table_opm;
	// PG sampler table.
	std::vector<Ref<SiOPMWaveSamplerTable>> sampler_tables;

	void reset_all_user_tables();
	void register_wave_table(int p_index, const Ref<SiOPMWaveTable> &p_table);
	Ref<SiOPMWaveSamplerData> register_sampler_data(int p_index, const Ref<SampleData> &p_data, bool p_ignore_note_off, int p_pan, int p_src_channel_count, int p_channel_count);
	...
	void set_stencil_custom_wave_tables(std::vector<Ref<SiOPMWaveTable>> p_tables) { _stencil_custom_wave_tables = p_tables; }
	void clear_stencil_custom_wave_tables() { _stencil_custom_wave_tables = std::vector<Ref<SiOPMWaveTable>>(); }
	void set_stencil_pcm_voices(std::vector<Ref<SiMMLVoice>> p_tables) { _stencil_pcm_voices = p_tables; }
	void clear_stencil_pcm_voices() { _stencil_pcm_voices = std::vector<Ref<SiMMLVoice>>(); }
