...
sion::String SiOPMOperator::_to_string() const {
	sion::String params = "";
	...
	params += "note=" + sion::String(is_pitch_fixed() ? "yes" : "no") + ", ";

	params += "ssgec=" + itos(_ssg_type) + ", ";
	params += "mute=" + itos(_mute) + ", ";
	params += "reset=" + sion::String(_envelope_reset_on_attack ? "yes" : "no");
...
SiOPMOperator::SiOPMOperator(SiOPMSoundChip *p_chip) {
	...
	_feed_pipe = new SinglyLinkedList<int>(1, 0, true);
