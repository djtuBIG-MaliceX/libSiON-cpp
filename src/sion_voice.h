...
public:
	...
	static std::shared_ptr<SiONVoice> create(SiONModuleType p_module_type = (SiONModuleType)5, int p_channel_num = 0, int p_attack_rate = 63, int p_release_rate = 63, int p_pitch_shift = 0, int p_connection_type = -1, int p_wave_shape2 = 0, int p_pitch_shift2 = 0);

	//

	std::string get_name() const { return _name; }
	void set_name(const std::string &p_name) { _name = p_name; }

	void set_params(std::vector<int> p_args);
	void set_params_opl(std::vector<int> p_args);
	void set_params_opm(std::vector<int> p_args);
	void set_params_opn(std::vector<int> p_args);
	void set_params_opx(std::vector<int> p_args);
	void set_params_ma3(std::vector<int> p_args);
	void set_params_al(std::vector<int> p_args);

	std::vector<int> get_params() const;
	std::vector<int> get_params_opl() const;
	std::vector<int> get_params_opm() const;
	std::vector<int> get_params_opn() const;
	std::vector<int> get_params_opx() const;
	std::vector<int> get_params_ma3() const;
	std::vector<int> get_params_al() const;

	std::string get_mml(int p_index, SiONChipType p_chip_type = (SiONChipType)-1, bool p_append_postfix = true) const;
	int set_by_mml(std::string p_mml);

	std::shared_ptr<SiOPMWaveTable> set_wave_table(std::vector<double> *p_data);

	std::shared_ptr<SiOPMWavePCMData> set_pcm_voice(const Variant &p_data, int p_sampling_note = 69, int p_src_channel_count = 2, int p_channel_count = 0);
	std::shared_ptr<SiOPMWavePCMData> set_pcm_wave(int p_index, const Variant &p_data, int p_sampling_note = 69, int p_key_range_from = 0, int p_key_range_to = 127, int p_src_channel_count = 2, int p_channel_count = 0);
	std::shared_ptr<SiOPMWaveSamplerData> set_sampler_voice(const Variant &p_data, bool p_ignore_note_off = false, int p_channel_count = 2);
	...
	void set_sampler_table(const std::shared_ptr<SiOPMWaveSamplerTable> &p_table);
	...
	std::shared_ptr<SiONVoice> clone();
